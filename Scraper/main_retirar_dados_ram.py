    ...
    try:
        ...
        if url != "":
            ...
            for caracteristicas in specs:
                if "Capacidade:" in caracteristicas.text:
                    capacidade = caracteristicas.text
                if "Tipo:" in caracteristicas.text:
                    tipo = caracteristicas.text
                if "Velocidade:" in caracteristicas.text:
                    velocidade = caracteristicas.text
                if "Latência:" in caracteristicas.text:
                    latencia = caracteristicas.text
            ...
            cursor.execute(f''' INSERT INTO "ram" VALUES("{nome}", "{preco}", "{capacidade}", "{tipo}", "{velocidade}", "{latencia}", "{imgnome}", "{id}","{url}")''')

            conn.commit()

            caract = f' "{nome}", "{preco}", "{capacidade}", "{tipo}", "{velocidade}", "{latencia}", "{imgnome}", "{id}","{url}" '
            todos = ""

            #ver se existe primeiro o ficheiro
            if not "ram_caracteristicas.txt" in os.listdir():
                with open("ram_caracteristicas.txt", "w") as file:
                    pass
            with open("ram_caracteristicas.txt", "r") as file:
                todos = file.read()
            todos+=f"\n{caract}"
            with open("ram_caracteristicas.txt", "w") as file:
                file.write(todos)
